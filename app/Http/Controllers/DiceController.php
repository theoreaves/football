...
//        $offensePlays = OffensePlay::where('code', '!=', 'PRESSURE')->get();
    ...
    public function resolve(Request $request, Game $game, GamePlayEngine $engine)
    {
        $data = $request->validate([
            ...
            'offense_play_id' => [
                'exclude_unless:play_type,NORMAL',
                'required',
                'integer',
                'exists:offense_plays,id',
            ],
            'defense_play_id' => [
                'exclude_unless:play_type,NORMAL',
                'required',
                'integer',
                'exists:defense_plays,id',
            ],
        ...
        $offensePlay = OffensePlay::find($data['offense_play_id'] ?? 0);
        $defensePlay = DefensePlay::find($data['defense_play_id'] ?? 0);
...
//                        'Result' => $resolved['result'],
        ...
        try{
            ...
            Log::error('Error attaching input data to resolved result: ' . $e->getMessage());
        }
